...
test-e2e *FILTER:
    cargo test --features cli,mcp,embed-ollama,embed-openai-compat,summarize-openai,rerank-voyage --test e2e {{ FILTER }}
...
test-e2e-valkey-cluster *FILTER:
    ...
    NIDUS_E2E_REDIS_URL="$(./scripts/e2e-services.sh cluster-url)" \
        cargo test --features cli,mcp,embed-ollama,embed-openai-compat,summarize-openai,rerank-voyage --test e2e cluster::{{ FILTER }} -- --ignored --test-threads=2
