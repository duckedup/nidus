...
# Checked here so a build failure fails with this message instead of `code ingest`
# surfacing as a clap "unrecognized subcommand" error further down.
if ! cargo run --quiet --bin nidus -- --version >/dev/null 2>&1; then
  echo "docs-index: cannot build a nidus binary (cargo run --bin nidus failed)" >&2
  exit 1
fi
...
cargo run --quiet --bin nidus -- code ingest . \
...
printf '[{"id":"docs-index.digest","attrs":{"digest":{"Str":"%s"}}}]' "$digest" \
  | cargo run --quiet --bin nidus -- upsert --dir "$store" meta >/dev/null
