...
description: Wire a nidus store into an agent so memories are written and recalled without the model having to remember to ask — a session-start hook that recalls context, a stop hook that writes back what was learned.
---
...
you. The plain `/search`, `/recall` and `/list` routes do not — so a hook querying
those directly will see expired memories unless it adds the `nidus.expires_at`
predicate itself. The examples above use `/recall`, so add that guard if you rely on
TTL from a hook — this is the exact predicate the MCP tools AND into every query:
