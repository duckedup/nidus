...
| `sweep_expired` | `fn sweep_expired(&mut self) -> Result<u64>` | Delete every entry across every collection whose `nidus.expires_at` has passed, then compact to reclaim the rows, in one call. Returns the number of entries deleted. Available in every build, no feature flag. |
