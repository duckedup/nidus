...
Three things about Python make this the highest-risk file in the SDK, so all three are
...
* There is no float attribute type at all. A ``float`` is refused loudly rather than
  truncated, because a silently-floored measurement is worse than a failed upsert —
  floats belong in the vector, not in attrs.
...
#: ``{"Str": …}``, ``{"Int": …}``, ``{"Bool": …}``, ``{"List": [...]}``, or the bare
#: JSON string ``"Null"`` (a string, not an object — that asymmetry is the wire's).
...
AttrInput = Union[Value, str, int, bool, list[str], None]

#: A :data:`Value` decoded back to a plain Python value.
DecodedValue = Union[str, int, bool, list[str], None]
...
def encode_value(value: AttrInput) -> Value:
    ...
    Raises ``TypeError`` for a float, a non-string list element, or any other
    unsupported type; ``ValueError`` for an integer outside ``i64``.
    """
    ...
    if isinstance(raw, float):
        # Its own branch so the message names the actual problem instead of listing types.
        raise TypeError(
            f"nidus has no float attribute type, got {raw!r} "
            "(floats belong in the vector, not in attrs)"
        )
