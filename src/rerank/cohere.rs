...
/// Cohere's documented per-request document cap.
...
fn scatter(
    ...
    if results.len() != len {
        return Err(RerankError::Decode(format!(
            "Cohere returned {} scores for {len} documents",
            results.len()
        )));
    }
    for d in results {
        if d.index >= len {
            return Err(RerankError::Decode(format!(
                "Cohere returned out-of-range index {} for {len} documents",
                d.index
            )));
        }
...
mod tests {
    ...
    fn body_shape() {
        ...
        assert!(b.get("return_documents").is_none());
    }
    ...
    #[tokio::test]
    async fn rerank_hits_v2_endpoint_and_returns_input_order() {
